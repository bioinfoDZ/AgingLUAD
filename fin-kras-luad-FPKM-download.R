...
mut <- readRDS('mut_maf.rds')



#download clinical data... maybe hold off on this for now...





#try to select only the kras cases...
mutmat <- maftools::mutCountMatrix(maf = mut, includeSyn = F)

# kras <- mutmat['EGFR',]
kras <- mutmat['KRAS',]
krasmuts <- t(kras)
krasmuts <- krasmuts[krasmuts[,1] > 0,]
krasmuts <- names(krasmuts)
rm(kras, mutmat, mut)

krasmutpatients <- substr(krasmuts, start = 1, stop = 12)
...
#get patients with kras muts
cdx <- cdx[cdx$patient %in% krasmutpatients,]
